...
#include <iomanip>
#include <cmath>
#include "./header/Gauss_Laguerre_Nodes_and_Weights.hpp"
#include "./header/Gauss_Laguerre_Nodes_and_Weights_3.hpp"
...
//
//        Gauss_Laguerre_Nodes_and_Weights_3(nodes, weights);
//        sum     =       0.0;
//        for (int k=0; k<3; ++k) {
//                sum+=weights[k]*cos(0.5*M_PI*nodes[k]);
//        }
...
//
        ...
        for (int k=1; k<=128; ++k) {
                sum     =       0.0;
                Gauss_Laguerre_Nodes_and_Weights(k, nodes, weights);
                for (int j=0; j<k; ++j) {
                        sum+=weights[j]*exp(-0.5*nodes[j]*nodes[j])*sin(nodes[j]);
                }
